...
class _GrafikDonasiScreenState extends State<GrafikDonasiScreen> {
  ...
  List<BarChartGroupData> _makeData(List<KelolaDonasi> list) => list.asMap().entries.map((e) {
        return BarChartGroupData(
          x: e.key,
          barRods: [BarChartRodData(toY: e.value.donasiTerkumpul.toDouble(), width: 16)],
        );
      }).toList();
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<KelolaDonasi>>(
        ...
        builder: (c, s) {
          ...
          final data = s.data!;
          final maxY = data.map((e) => e.donasiTerkumpul).reduce((a, b) => a > b ? a : b).toDouble() * 1.2;
          return Padding(
            padding: EdgeInsets.all(16),
            child: BarChart(BarChartData(
              maxY: maxY,
              barGroups: _makeData(data),
              titlesData: FlTitlesData(
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(showTitles: true, getTitlesWidget: (v, _) {
                    final idx = v.toInt();
                    return Text(idx < data.length ? data[idx].nama : '');
                  }),
                ),
                leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: false)),
              ),
